...
class ExamplePLP(io.ParametricLP):
    ...
    def generate(self, u, w): ##
        ...
        c=[[u*(w**2)],[0.0]] ## cost function should be defined to the best convenience of the model
        #c = [[torch.cos(w + (u**2)/2)],
        #     [torch.sin(w + (u**2)/2)]]
...
weights_pred = io.inverse_parametric_linprog(u_train,io.tensor(np.transpose(np.concatenate((np.expand_dims(u_train_sal_model[:,0],axis=0),np.expand_dims(np.squeeze(x_train),axis=0)),axis=0))),plp_learn,max_steps=int(sys.argv[3]),callback=io.inverse_parametric_linprog_step_printer())
